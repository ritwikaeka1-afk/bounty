...
type Tab = "home" | "browse" | "post" | "tasks" | "profile" | "confirmation";
...
export default function BountyApp() {
  ...
  return <main className="bounty-app">
    ...
    {tab === "confirmation" && confirmation && <section className="confirmation"><span>✓</span><p className="eyebrow">BOUNTY PUBLISHED</p><h2>{confirmation.title}</h2><strong>✦ {confirmation.reward} recommended credits</strong><p>Your bounty is live for verified UCLA students. Review offers by skills and experience, then accept one to hold the credits in escrow.</p><div><button className="primary" onClick={() => setTab("tasks")}>View my tasks</button><button className="text-button" onClick={() => setTab("browse")}>Browse marketplace →</button></div></section>}
...
const label = (tab: string) => ({ home: "Home", browse: "Browse", post: "Post", tasks: "My tasks", profile: "Profile" }[tab] ?? tab);
